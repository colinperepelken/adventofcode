...
const solve = () => {
    ...
    const minDirectorySizeToDelete = requiredFreeSpace - currentFreeSpace
    
    let currentMin = Infinity
    for (const directory of Object.values(fileSystem)) {
        if (directory.size >= minDirectorySizeToDelete && directory.size < currentMin) {
            currentMin = directory.size
        }
    }
